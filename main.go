...
// loggingMiddleware logs all incoming requests and their response status codes
...
func loggingMiddleware(next http.HandlerFunc) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		log.Printf("Response: %s %s - Status: %d - Duration: %v", r.Method, r.URL.Path, responseWriter.statusCode, duration)
...
func loadEndpoints() {
	...
	for _, file := range files {
		...
		fmt.Println("Loading function: " + funcName)
...
func httpHandler(w http.ResponseWriter, r *http.Request) {
	...
	if CallType != "" {
		...
		if !ok {
			...
			fmt.Fprintln(w, "Not registered function: "+CallType)
...
func main() {
	...
	http.HandleFunc("/re_load_func", loggingMiddleware(re_load_func))

	http.HandleFunc("/", loggingMiddleware(httpHandler))

	http.HandleFunc("/authTkn", func(w http.ResponseWriter, r *http.Request) {
