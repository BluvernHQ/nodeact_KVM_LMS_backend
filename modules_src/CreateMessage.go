...
func CreateMessage(w http.ResponseWriter, r *http.Request, db *mongo.Client, authClient *auth.Client) {
	...
	data.Sender = token.UID
	data.TimeStamp = time.Now().Format(time.RFC3339)
	collection = db.Database("KVM").Collection("Messages")
