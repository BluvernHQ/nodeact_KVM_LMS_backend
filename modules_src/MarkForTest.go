...
type TestMarks struct {
	...
	UIDs      []map[string]interface{} `json:"UIDs"`
	ForDate   string 				   `json:"ForDate"`
	MarkedAt  string                   `json:"MarkedAt"`
}

func MarkForTest(w http.ResponseWriter, r *http.Request, db *mongo.Client, authClient *auth.Client) {
	...
	collection = db.Database("KVM").Collection("TestMarks")
	var docs []interface{}

	for _, uidObj := range data.UIDs {
		doc := bson.M{
	...
	json.NewEncoder(w).Encode(map[string]interface{}{
		"message": "Test marks inserted successfully",
	})
}
